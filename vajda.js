...
class Buff extends Consumable {
    ...
    get isSelected() {
        return ConsumablesManager.getSelectedBuffs().some(b => b.id === this.id) || this.isSelected_
    }
...
window.ConsumablesManager = {
    ...
    isScheduledRefill: function(jobCount) {
        const isScheduled = this.isOptimized && this.getRefillsLeft() > 0 && this.jobsLeftInRound <= this.schedule.at(-1)
